...
import { Product } from '@/types';
import ProductCard from '../common/ProductCard';
import { SearchX, Loader2, Star } from 'lucide-react';
import { Button } from '@/components/ui/button';
...
interface ProductGridProps {
  ...
  onViewModeChange?: (mode: 'grid' | 'list') => void;
  className?: string;
  featuredFilter?: boolean; // Add featured filter prop
}
    ...
    return (
      <div className={className}>
        <div className="text-center py-12">
          <SearchX className="w-12 h-12 text-gray-400 mx-auto mb-4" />
          <h3 className="text-lg font-semibold text-gray-900 mb-2">
            {featuredFilter ? 'No featured products found' : searchQuery ? 'No products found' : 'No products available'}
          </h3>
          <p className="text-gray-600 text-sm mb-4">
            {featuredFilter
      ...
      <div className={
        ...
        {products.map((product, index) => (
          <ProductCard
            ...
            showFeaturedBadge={true} // Show featured badge on product cards
          />
        ))}
      </div>
